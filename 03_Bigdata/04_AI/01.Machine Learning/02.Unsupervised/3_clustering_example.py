...
user_product_dic = {}
# 상품 코드를 키로, 상품 코드의 셋을 밸류로 갖는 딕셔너리
product_user_dic = {}
...
def analyze_clusters_keywords(labels, product_id_name_dic, user_product_dic, id_user_dic):
    ...
    print(Counter(labels)) # 리스트의 각가의 값 별로 누적 현황 확인
    cluster_item = {}
    ...
    for cluste_id, product_name in cluster_item.items():
        ...
        #구매한 상품들의 상품명안에 가장 자주 나타나는 단어 20개를 역순으로 출력
        print("cluster_id:", cluste_id)
...
def analyze_clusters_keywords_bigram(labels, product_id_name_dic, user_product_dic, id_user_dic):
    ...
    for cluster_id, product_name in cluster_item.items():
        ...
        #OF를 공백으로 리플레이스하고 스페이스 혹은 탭으로
        #스플릿하여 키워드로 분해한 뒤 연속되는 두 키워드를 합쳐서
        #하나의 키워드를 생성
        bigram = []
...
for line in open('online_retail_utf.txt'):
    ...
    #영국에서 구매한 사용자만 고려하므로, 국가가 united kingdom이
    #아닌 경우엔 무시
...
#데이터구조를 다 채웠으므로 각 사용자들이 구매한 상품 가짓수로 리스트 생성
...
#skewness: 값의 쏠림 정도, 양의수:정규분포에서 왼쪽으로 쏠리는 경우
#음의수: 오른쪽으로 쏠림
# kurtosis: 정규분포상에 값의 쏠림을 나타내는 다른 지표, 값이 클 수록
#쏠림현상이 강하다.
#구매한 상품의 가짓수가 1인 사용자의 사용자 ID를 검색
min_product_user_li = [k for k,v in user_product_dic.items() if len(v) == 1]
#마찬가지로, 구매한 상품의 가짓수가 600개 이사인 사용자의 사용자 ID를 검색
...
for k in range(2,9):
    ...
    # s = b -a / max(a,b)
    # a: 하나의 샘플과 그 샘플과 같은 클러스테이 있는 다른 샘플 간의 거리 평균
